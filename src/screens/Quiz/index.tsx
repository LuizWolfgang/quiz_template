...
export function Quiz() {
  ...
  async function shakeAnimation() {
    ...
    shake.value = withSequence(
      ...
      withTiming(0, undefined, (finished => {
        'worklet'; //mudando de thred para funcionar
        if(finished) {
  ...
  .onUpdate((event) => {
    ...
    if(moveToLeft) {
      cardPosition.value = event.translationX // movendo cartao somente se for para direita
    }
  })
  .onEnd((event) => {
    if(event.translationX < CARD_SKIP_AREA) {
      runOnJS(handleSkipConfirm)(); //chamando funcao atraves do gesto
    }
